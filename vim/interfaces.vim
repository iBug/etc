...
syn keyword interfacesOpta address gateway netmask dns-nameservers broadcast metric pointopoint media hwadress mtu hostname leasehours leasetime vendor client bootfile server hwaddr provider frame netnum endpoint local ttl network 
syn match interfacesOptaa /\(bond\)-\w\+/
...
syn match interfacesIP6 /\([0-9a-fA-F]\+:\)\{1,7}\(\(:[0-9a-fA-F]\+\)\{0,7}\|:\)\?\(\/\d+\)\?/ contains=interfacesIP6
"MAC addresses
